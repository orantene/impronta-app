...
| PackageEditor | not built | `Package` is a kind and is created, listed and edited as one item (W02, W03); Composition, allocation and the guest preview need a components model that does not exist (D-POS-53). |
| P01_PackageConfigure, P02_ComponentUnavailable, P03_PackageAfter, P06_PackageRefund | not built | The till configuring, substituting, issuing and refunding a package's components; no components model, no screens made only of disabled controls (D-POS-53). |
