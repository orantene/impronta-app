...
| POSWaitlistOffer (T08) | **partial** | `POSWaitlistOffer.live.png` | Party waitlist on Tables is `partyWaitlistJoin` / `Notify` / `Seat` / `Leave` (D-POS-78). Notify records `notified_at` and returns `channel: none` unless an email was accepted. Not wired: SMS offer text. |
...
| POSTableChange (T12) | **matched** | `POSTableChange.live.png` | The three cards: Move (wired), Join (disabled: a join is decided at seating), Merge (disabled: no merge command). |
| POSMoveParty (T13) | **matched** | `POSMoveParty.live.png` | `T1 › Move`, the grid of tables with Free / Too small / Too large / Occupied / Needs reset / Held / Blocked, `Goes with them` ($x check · n kitchen ticket), `T1 after · Needs reset`, Why (disabled: not recorded), Back · `Move to X` (`tablesMoveVisit`, the check and tickets follow). Differs: no `Next on X · Nothing until 21:30` line (the next booking is on the list view). |
| POSJoinTables (T15) | **partial** | `POSSeatParty.live.png` | A pair is offered on the seat sheet (`T2 + T3 · joined · seats 3–4`) and seats one visit across both (the proven `joinedSpaceId`); joining a second table to an already seated party is disabled with its reason. |
| POSMergeChecks (T16) | **not wired** | `POSTableChange.live.png` | No merge command; the card is disabled over its sentence. |
| POSChangeServer (T17) · POSExtend (T22) | **not wired** | `POSTableActions.live.png` | Disabled rows on the card, each with its one sentence. |
...
| R06_LayoutsShareCapacity | **partial** | (none) | `layoutActivate` never writes capacity pools. Activate is a separate call from layout upsert. |
...
| W13_LayoutEditor | **partial** | (none) | Settings › Layouts: canvas items and `layoutUpsert` / `layoutActivate`. Floor tiles use the active layout when coordinates exist. |
| W14_ServicePeriods | **partial** | `W14_ServicePeriods.live.png` | Settings › Service periods writes `servicePeriodUpsert`. Pacing and money fields stay disabled with their sentence. |
| W15_PrepStations | **partial** | (none) | Settings › Prep stations writes `prepStationUpsert` / delete. Dispatch toggles stay disabled. T26 Fire starters/mains/dessert is `prepFireCourse`. |
| W23_LocationsZones · W24_BookingPolicies | **partial** on W23 | (none) | W23 is `locationsList` / `locationUpsert` / zones. W24 booking policies stay not wired. |
...
| Q02 · Q03 · Q04 · Q06 · Q07 | **partial** | (none) | Guest QR at `/visit/[token]/menu` and `/visit/[token]/share` (390). Browse, add line, submit, accept substitute, pay-my-share mints a Package 1 link. |
...
Card: Change server, Extend time, Block table, Split the check. Chooser:
Join tables (for a seated party), Merge checks. Seat sheet: Server, No-show.
