...
| W46_ProjectAgreement (`live.png`, `live-proposed.png` with a sent v4 seeded on the isolated database and reverted) | partially | The version strip (`v3 · accepted (current)`, proposed, superseded), `Propose change` (a link to the conversation when the slot is free, disabled with the refusal otherwise), the accepted card (`Binding`, the assignment lines left, Value · Coordinator fee · Cancellation · Usage rights right), the proposed-change card with its sentence, the prior-versions sentence. **2026-09-11 (wire-scheduling):** on a DRAFT proposal the card carries `Send vN` and `Discard proposal`, live through `amendmentSend` / `amendmentDiscardAction` with the offer's version and the conversation's version (`inquiries.version`, now on the record as `inquiryVersion`) as the lock; `conflict`, `not_draft`, `not_found` are the engine's sentences under the buttons. A SENT proposal keeps `Remind` (disabled, D-POS-36) and `Withdraw on the conversation` (the engine discards drafts only). Cancellation and usage rights read "Not recorded on the agreement". The fixture has one accepted version, so `live.png` is unchanged; the draft state was not seeded (it needs a draft offer on the conversation). |
| W47_ProjectMilestones | partially | `Milestones & deliverables`; one row per deliverable: title with revisions used, when, amount, pill, `Request changes` / `Approve (client)` on a submitted one (live, the engine's `requestRevision` / `approveDeliverable` with every refusal as a sentence). One list, not two: the same table is the milestone and the deliverable. **2026-09-11 (wire-scheduling), closing D-POS-37:** `Add milestone` is live (title · due date, the engine's `createDeliverable` through `addProjectMilestone`); the amount is `booking_deliverables.amount_cents`, drawn as money and edited in place (`setDeliverableAmountAction`; zero reads as the dash, never $0); each open row has `Upload` / `Replace file` through the conversation's signed-upload path and `attachDeliverableFileAction` (D-POS-79), and the Files & activity tab lists the milestones that carry a file. Differs: no `Edit` of a title or date (no writer); the board's `M1 · Agreement signed · Paid 5 Sep` links a milestone to a payment, which no row records. `live.png` is the fixture project with one milestone added on the isolated database for the frame (`Final edits`, $500.00) and deleted after. |
| W48_ProjectTeamReplace | partially | The Assignments rows (name · role/date/units · fee and charge · `Assigned` · `Replace…`), `Assign from People › Bookable` (to the conversation's lineup), the sheet with the five impact rows from the record and the note. **2026-09-11 (wire-scheduling), closing D-POS-38:** `Replacement` is a select over the workspace's roster (`loadReplacementCandidates`, every non-removed person profile) and `Replace and invite <name>` calls `projectReplaceTalentAction` under one operation key; `talent_unavailable`, `already_started`, `not_found` are the engine's sentences in the sheet. Differs: the board's `Loba free ✓` on the schedule row is the engine's answer after the click, not read before it; no invitation state exists, so the row reads `Assigned` before and after; a person typed on the offer without a profile cannot be replaced and the button says so; the outgoing name stays on the row after a replacement until the engine rewrites the snapshot (D-POS-81, engine finding). `live.png` is the sheet open with the roster in the select; not confirmed (live-tenant QA writes nothing it does not need to). |
...
| W50_ProjectClose | partially | The sheet from `More ▾` and from the primary action when the project is closable: Outstanding (client money · work · talent), the four choices each with its verdict (`Complete` live via `closeBookingAction`, `Cancel` live via `cancelBookingAction`), `Back`, `Complete project` / `Cancel project…` / `Archive project` / `Reopen project`. **2026-09-11 (wire-scheduling), closing D-POS-39:** `Archive` is live for a completed or cancelled project (`projectArchiveAction`) and `Reopen later` for an archived one (`projectReopenAction`); `closeOptions` says why not before the click (`not_closed`, `not_archived`, `already_archived`) and the engine's `not_archivable` / `not_reopenable` / `conflict` after it. Every engine refusal is a sentence. The fixture project is confirmed and owes money, so Cancel is the only live choice in `live.png`; Archive reads `Only for closed projects`; not pressed. |
...
| W44_ClientCollectSheet | partially | `Collect from <name>`, Unpaid records with checkboxes (the first ticked), Allocation (Selected · Tip · Pass or credit · Receipt), the note, `Cancel` · `Send payment link` (disabled, D-POS-27) · `Continue to payment · $X` (opens the counter on that one sale). One record at a time (D-POS-40). |
...
| POSPaymentLink | not wired | The Links destination: the board's frame over one sentence and the board's own rule (D-POS-42). |
...
New project · Owner filter and column · Request approval / Remind · Edit
a milestone's title or date · Record bank transfer · Send payment link
(record, sheet, till) · Refund when nothing was collected · Edit client ·
Send vN / Discard proposal on the POS Projects destination (O06; the
workspace's W46 is live) · Links. Decisions D-POS-35 to D-POS-43 in
`docs/plans/program/pos/decisions.md`; wired by Package 2 on 2026-09-11:
Add milestone, milestone amount and file, Replacement and Replace and
invite, Archive and Reopen, Send vN / Discard proposal on W46 (D-POS-76 to
D-POS-81).
