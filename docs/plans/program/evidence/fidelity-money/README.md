...
| W56_ApprovalReview | **not wired** | (no live frame) | The engine has no per-action limit and no approval request to review (People's W22 evidence records the same gap), so there is nothing to draw the dialog over. Recorded as D-POS-59 rather than built as a dialog that would approve nothing. |
