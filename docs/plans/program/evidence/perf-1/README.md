...
| 7 | `perf(bundle)` | Leaf imports instead of the `@/lib/site-admin` barrel on the root and admin paths (`tagFor`, `resolveDesignTokens`, `DEFAULT_PLATFORM_LOCALE`, `homepageTemplate`); `EditChromeMount` keeps every server gate in place (storefront path, tenant host, membership of THIS host's tenant, edit cookie, home-role resolution, legacy-adapter fallback, locale-filtered freeform probe) and imports the three builder adapters at runtime, after those gates, instead of statically; `EditChrome` itself is a `next/dynamic` boundary. | static import graph (`tools/import-graph.mjs`): from `app/layout.tsx` and the admin layout/page/POS/messages routes, `sections/registry.ts`, `builder-node/render.tsx`, `edit-chrome/edit-chrome.tsx` NOT reached (before: all reached). Bundle figure after this commit: **owed to the next build** (this session's one build predates it) |
