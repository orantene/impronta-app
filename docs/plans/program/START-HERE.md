...
C06 restaurant paths, C01-CUS deposit-requested, C09 class paths + C09-DIFF last-seat sold-out, C02-CUS last-resource + couples set, C02-DIFF, C12-CUS $0 ticket, C12-OP door Admit, C12-DIFF pay-at-door cash settle, C26-OP pickup handoff, C07-OP tab collect-at-close, C07-CUS guest check, and C08-CUS directory inquiry submitted are recorded. Next: Stripe test deposit collect, C02-OP assign (Calendar New booking does not pick therapist + room), C08-OP assign models / send offer, C01-OP balance, or remaining representatives (C13, C24, C27, C31). Do not merge #1934 as complete. Do not start P9. Do not re-apply production migrations. Do not reset the qa-journeys branch.
