...
| C08-CUS directory inquiry | **Pass** as inquiry *submitted* on qa-journeys `/directory` chat + DB (re-run 2026-09-09T08:50Z after D-028 cue queue). Evidence: `docs/plans/qa-evidence/C08-CUS/directory-inquiry.md`. Offer not accepted. |
...
| P0-05 db:check + stale docs | Remote applied `20261230000200`–`00600` on `pluhdapdnuiulvxmyspd`. **Do not re-apply to production.** The full local range is `20261230000200`–`20261230002200` (21 files); everything after `00600` is on this branch and on qa-journeys only, not production. `20261231*` is reserved for the money/capacity hardening work and has no files yet. `db:check` cannot see qa-journeys at all (D-012); use `journeys:probe` |
