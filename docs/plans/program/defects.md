...
| D-005 | high-risk | awaiting external verification — P1-01 | capacity | Concurrency proof refuses unless `CAPACITY_PROOF_ISOLATED=1`. qa-journeys has `reserve_capacity` and a 12-place pool; sequential SQL 13×1 yielded 12 holds + 1 sold_out and was released. 200 concurrent HTTP calls not run (no `service_role`). Never production. | `verify-capacity-concurrency.mjs` |
