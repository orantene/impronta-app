...
| **P1.5** | ~~`admissions` table~~ **NOW EVENTS & TICKETING'S** (`20261229000360`). Mine: the `check_in` RPC, token derive/verify, `order_lines.session_id`. | P1.2, Events' table | Table exists in production, `has_table_privilege` false for `anon` and `authenticated` on every write verb and true for `service_role`; token round-trips, a tampered token fails, a wrong-purpose token fails. |
