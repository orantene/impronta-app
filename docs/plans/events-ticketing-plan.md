...
## 6a-iii. THE TIER ↔ POOL CONTRACT — written once, 2026-09-05 (CEO-confirmed)

Agreed between Events & Ticketing, Sessions & Classes, the Sessions/Events/Reservations Director and the CEO. This section is the record; every reader and writer below refers to it, none re-derives it.

1. **A tier's pool key is `talent_offering_variants.pool_key`, a stored column.** It is set ONCE at tier creation by `poolKeyFor(label)` (`lib/events/tiers.ts`, via `newTierRow`) and **never recomputed** — a rename is an `UPDATE` of `label` only. A tier renamed from "GA" to "General admission" keeps its pool and its sold seats. Not the uuid, not a re-derived slug: either would orphan seats the first time the derivation changed. A variant with no `pool_key` is not a tier and gets no pool. Readers that resolve on it today: the door sale, the mint, `loadWorkspaceEvents`, the public event page, `loadDoorTiers`, `addTier`'s duplicate check.
2. **A pool is per session: `capacity_pools(subject_kind = 'session_tier', subject_id = <session id>, pool_key)`.** Units are **per-night input from the venue at scheduling time** — `createSessionWithPools(session, [{ poolKey, units }])` — and **the tier row carries no units.** A tier is not a table: the same "VIP table" tier is 6 tables on Friday and 4 on Saturday; "GA" is 300 in the main room and 120 in the garden. A units column on the tier would be one number pretending to be every night's. The durable reason (Sessions): a seat count read off the tier at scheduling time is a *snapshot*, and the moment either number is editable the tier says 300 while the night says 250 and nothing on screen says which is authoritative. Per-night input has no such gap because the pool is the only number that ever existed.
3. **One creator.** `createSessionWithPools()` in `src/lib/sessions/` (Sessions & Classes) is the only code that creates `session_tier` pools; the cron `materialise-sessions` (#1621, the existing engine with no human door) is refactored onto it. A tier added after a session exists gets its pool through the sibling `ensureSessionPools(sessionId, [{ poolKey, units }])`. Nobody calls `upsert_capacity_pool` for `session_tier` outside those two. **Duplicate keys are refused at plan time**: `upsert_capacity_pool` is `ON CONFLICT DO UPDATE SET units_total = EXCLUDED`, so `[{ga,300},{ga,6}]` silently yields ONE pool of 6 — a venue that meant 300 opens the doors to 6 and nothing downstream can tell. `createSessionWithPools` refuses a duplicate `poolKey`; `addTier` refuses a duplicate `pool_key` on one offering, so a plan built from tiers cannot contain one. Until a night has a pool for a tier, that tier is unsellable for that night and every surface says so rather than hiding it.
3a. **Reschedules and per-leg holds — Capacity's #1855, applied 2026-09-06.** `extend_capacity_hold(p_allocation_ids uuid[], p_ttl_seconds int)` exists: `hold` → extended, `committed` → skipped (the complete set is the safe set to pass), `released` → **raises `CP016`** (the one input that could resurrect a resold seat). It **can shorten** a hold — a reschedule to an earlier night should — so it returns `{extended, shortened, unchanged, skipped_committed, requested}`, never void: an applier that moves a night LATER and sees a non-zero `shortened` has cut seats short, not protected them — **a refusal-worthy condition, not a metric** (Sessions): the applier treats it as a failure of the move and surfaces it to the operator who just moved the night, never logs it beside a success. One label covering two states is where a defect hides; splitting `extended` from `shortened` before anyone built on it is why this one costs nothing. `reserve_capacity_batch` takes a per-leg `ttl_seconds` with the batch value as fallback and the pool clock last; existing callers are byte-identical. The 7-day `CP007` ceiling is untouched: a product rule, not a limit to route around. Orders' #1836 derives a door hold to the session's END from `lines[].sessionId` (no TTL passed by the picker), clamps to the engine's cap, and refuses an already-ended session; the single source for the cap is `lib/capacity/hold-ttl-bounds.ts` (Orders), re-exported from `@/lib/capacity` by Capacity — every other copy is to be replaced by that import (a guard fails any bare `604800` elsewhere).
4. **The tier edit path is Events & Ticketing's.** Lowering a pool's `units_total` below what is already committed is refused by **Capacity's check inside `upsert_capacity_pool`**, called from the editor and never re-implemented in it. Capacity built it the same afternoon (#1769, applied): a refused shrink is SQLSTATE `CP015` / `capacity_floor_violated` with the floor in `DETAIL` — the editor shows that number; the floor is a **peak, not a sum** (`capacity_pool_committed_peak`), overbook counts in the ceiling, ancestors count via `pool_path`, and deactivation is deliberately not blocked.
5. **Acceptance runs through Sessions' scheduler screen**: an admin schedules a session, picks the event (`sessions.event_id`), and gives seats per `pool_key` for that night. The Events page's Sessions tab links to it and does not duplicate it.

Corrections folded in on the way to this text: the cron writer exists (#1621), so Sessions' gap is "an engine with no door", not "no engine"; a stacked PR's `files` shows its parent's migrations; a CEO ruling from memory (uuid key, units on the tier) was withdrawn against the record above — and the stronger fact, found by Sessions while preparing to comply: **`talent_offering_variants` has no seats or units column at all** (`…364` adds `pool_key`, sale window, min/max, `is_hidden`, `age_gate`, `description`, `seating_mode`, `space_group_id`), so "units come from the tier row" had nothing to read and would have been implemented as an invented column inside what was framed as a naming decision.
...
| Date | Entry |
|---|---|
| 2026-09-06 | **E5 built end to end as drafts, in the Director's order behind the owner's run**: #1816 step 1 (picker actions, card hop, seat-lost intents `…800`–`…802` + claimed executor), #1838 step 4 island (13/13 `session_picker` sites; never a buy control it cannot complete), #1845 step 4 page (`/events/<slug>` from the Creative Director's `festival.ts` under their venue-page ruling; pure view + loader; builder clamps; venue tokens; fixture renders for their DOM review), #1846 step 1b (pay at the door on Orders' #1836: `in_person`, NO TTL from us, hold derived to the session's END). Merged: #1818 mint hotfix (`…803`, `deb1931e8`). Open normal: #1839 (tolerant receipt page). |
| 2026-09-06 | **INCIDENT (mine)**: `…804` changed `receipt_for_code` to `SETOF jsonb` on production under the deployed page → `/r/<bogus>` 500 for ~4 min on both tenant hosts (PostgREST array vs scalar). `…805` restored the scalar; live 404 again. **Rule (Director)**: schema-first apply is for ADDITIVE schema; a change to the SHAPE of anything deployed code reads is CODE-FIRST — tolerant reader ships and is confirmed live (curl the surface BEFORE the apply), then the schema moves. `…806` (SETOF) applies only after the marker contains #1839. |
| 2026-09-06 | **INCIDENT (found by Reservations)**: the mint could never plan — `admissions_line_seq_uniq` was PARTIAL, `ON CONFLICT` → `42P10` on every settled order; `admissions` empty platform-wide was that. Fixed `…803` (non-partial; proof inside the migration; `indnullsnotdistinct = false` verified on the object). Rule: any `.upsert({ onConflict })` target must be a non-partial unique index/constraint; prove the real statement with an impossible tenant and require 23503. Static guards pin both targets. |
| 2026-09-06 | **Contracts settled**: hold TTL cap is 7 days (`CP007`, column CHECK) → pay-at-the-door offered only when the session ENDS within it, card beyond, with the sentence "Paying at the door opens closer to the date"; online pay-at-the-door closes at session start (the door's own sale covers walk-ups); reschedule extends holds via Capacity's `extend_capacity_hold` (hold→extend, committed→skip, released→raise, counts returned) called by Sessions' future applier in the same transaction; no disposable-email gate (the door finds a guest by name); a seat-lost refund carries a message to the buyer; refund intents are claimed before executed; Orders asked to derive their 30-day door clamp from the engine's 7-day cap. Live Stripe endpoint: `/api/webhooks/stripe` (`we_1U1rrP5C0mUEeRd1FXf7WWRL`), both order routes shims over one handler. |
