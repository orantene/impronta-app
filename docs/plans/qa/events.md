...
7. **Open `https://elpaisa.tulala.digital/events/noche-de-prueba` in a private window (signed out).** The page shows the title and `General admission · 150.00` and **no way to buy**; `https://elpaisa.tulala.digital/events` lists it. *Wrong:* a Buy / Add to cart control; a 404; the date shown in your laptop's timezone rather than the venue's (there is no date yet, so "Date to be announced" is correct).
