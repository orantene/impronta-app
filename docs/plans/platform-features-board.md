...
| **Orders 0.4** — `customers` table, backfill, `lib/customers/` | **MERGED as `63c98ffde` (PR #1513)**, structural gate 13m7s, all seven checks green. Migrations `20261228000140` + `…141` applied to production. Production verification owed. | 8 customers, 8 distinct emails, 6 correctly sharing one phone (measured by the manager; an earlier 7 here was mine and wrong). |
| **Orders 0.5** — `orders`, `order_lines`, convert writes an order, allocations by FK, function grants asserted | **MERGED as `a56a53bef` (PR #1514).** Migrations `…142`–`…145` on main and applied to production. Production verification owed once the pointer advances. | 838 lines, no TypeScript touched: the schema lands and nothing reads it. Convert parity proven at 50002¢ three ways (offer = order = line totals), house lane preserved, `offer_major_to_cents` asserted identical to the existing inline cast across 12 values including ties and negatives, inside the migration so they cannot drift. **0.4 and 0.5 are both on main — the Orders spine exists.** |
...
| D9 | **`.github/workflows/auto-apply-migrations.yml` exists on `main` and is DORMANT.** Activate it, or leave it off? Verified from the run log on `a56a53bef`: `SUPABASE_ACCESS_TOKEN and/or SUPABASE_URL not set — skipping auto-apply as a no-op`. All five recent runs are green and every one did nothing. **Activating it is two GitHub repository secrets and no code change.** It runs the same `apply-migration.mjs --apply-pending` the department already uses, file-by-file and idempotent, and its header explains it avoids `supabase db push` for exactly the drift reason this department hit. Activating would close the gap CLAUDE.md names as the cause of three silent-500 incidents; leaving it dormant keeps the manual step. **Either way CLAUDE.md needs a sentence**, because "there is no migration auto-apply yet" is true today and becomes false with two secrets. The Director did not edit CLAUDE.md unilaterally. | department-wide migration protocol | **open, owner action in GitHub Settings** |
