...
| `order_lines` → capacity allocations link shape | **Challenged, not ruled: `allocation_ids uuid[]` should probably be a join table `order_line_allocations`.** Orders and Capacity agree the shape between them and may overturn. | Postgres cannot FK an array element, so "no FK" is a consequence of the array, not a design choice. The independence argument does not need it: the reference points from Orders to Capacity, which is the correct layering direction, and Capacity never learns Orders exists. A join table adds referential integrity and makes refunds-by-line an index lookup instead of a scan. It also matches Capacity's `released` state, which records which line released how many units and when — a relationship with attributes is a table. |
| `inquiry_offer_line_items.source_service_id` is TEXT, not a uuid FK | **Manager's call, with the measurement on the record.** Orders proposed permanent cast-and-drop tolerance in `order_lines.offering_id`. Measured: the table has **zero rows** in production, and every writer stamps an offering id (`offerings-offer.ts:67`, `menu-order-offer.ts:62`, `inquiry-engine-offers.ts:910`, and the picker's own comment). 157 offerings exist. | The tolerance is for data that has never existed, and a drop-rate metric that reads zero forever tells nobody anything. With zero rows the tightening is a no-backfill `ALTER ... TYPE uuid USING ...` plus an FK, which catches a bad writer loudly at write time instead of silently at conversion time. Downstream tolerance for a broken upstream reference is the shape of the recorded lesson "copying the sibling pattern preserved the bug". Reasoning required in the PR body either way. |
