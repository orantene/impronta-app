...
| **Orders & Checkout** | **Set a tenant's words row to "Quote" and confirm the card follows** | **Narrowed 2026-09-03 — the card itself HAS been seen.** The manager signed in via `/api/dev/signin` on localhost (a dev-only endpoint, not a production login, so no password was typed into a login form) and captured a real order card rendering `ORDER · $42.50 · 2 items · "Order to pay"` in amber against a staged production order, cleaned up after. **Proven:** it renders on the canonical thread, figures read from the order rather than a stored label, the awaiting state is right, the four-hop wire delivers end to end. **NOT proven and why this line survives:** the screenshot predates the `loadTenantWords` wiring, so it shows "Order" — **which is the fallback and looks identical to a tenant whose words row says "Quote".** That is precisely the hardcode-wearing-a-parameter failure the same manager found in `orderCardView`, where a `noun` parameter existed and nothing ever passed one. Also unclicked: Pay now, Add line, and the client and talent surfaces. |
