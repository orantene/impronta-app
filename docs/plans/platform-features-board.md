...
**What replaced it, after BOTH of us reached it by a wrong route.** Reservations withdrew their own model, but the reason each of us gave was wrong, and they caught both.

**My stated clincher was wrong and does not discriminate between the designs.** I cited `hours-types.ts:65-68` rejecting `endMin > 1440` — true, and it correctly kills building on the *weekly hours* shape, which is still the reason Reservations is not built on the appointments engine. But it is not an argument for materialised occurrences: the alternative shape is `local_time` + `duration_minutes`, which is what `session_series` already uses (`duration_minutes int NOT NULL CHECK (> 0 AND <= 1440)`), and that crosses midnight perfectly well. **Recorded because a reason that sounds decisive and does not discriminate will be reused later on a case where it does not hold.**

**Their stated reason was also the wrong test.** "Nothing would ever foreign-key to a service-window occurrence" tests whether an occurrence is *referenced*. The right test is whether an occurrence needs to **vary**, and a restaurant's service varies constantly: closed 25 December, New Year's Eve 20:00 to 02:00 with last seating at 23:00, brunch only this Sunday. **A rule plus a resolver forces an exceptions mechanism, and an exceptions mechanism is a materialised occurrence with extra steps.** Sessions already has materialisation, `status='cancelled'`, the venue-timezone resolver and the calendar union; rebuilding all four elsewhere to avoid ~700 rows a year per venue is the worse trade.

**The shape that follows, and it avoids a second source of truth entirely.** A service window is a `session_series` row with `seats = 0` and `offering_id` null — both already legal on main (`seats int NOT NULL CHECK (seats >= 0)`, `offering_id` nullable) — materialised by Sessions' existing cron. Reservations' own table becomes a **policy sidecar keyed by `series_id`**: last seating, seating step, turn-time override, party bands, deposit and no-show rules, none of which belong on Sessions' table. One series table, one materialiser, one occurrence table.
