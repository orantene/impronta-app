...
**What replaced it is better than either position.** Reservations withdrew their own model an hour later with the stronger argument: **nothing would ever point at a service-window occurrence.** A class session is named by its tier pool and by every admission; a dinner service is named by nothing, because capacity sits on the band pool and an allocation runs `[T, T+turn)` — a turn floating inside the window, not the window itself. Materialising 365 unreferenced rows a year per venue is a second implementation of one concept. Reservations builds `venue_service_windows` shaped like `session_series` and **calls Sessions' `lib/sessions/recurrence.ts`** rather than copying it — one implementation of "a wall clock is not an instant", now load-bearing for two areas.
