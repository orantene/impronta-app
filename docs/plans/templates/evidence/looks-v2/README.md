...
**Correction (owner caught it):** the earlier captures showed the platform's LEGACY header (cream bar, icons) reading "AURA MODELS", not the Look's header. Two real causes, both fixed in this branch: (1) the QA tenant was not in `SITE_SHELL_TENANT_IDS` in this worktree's `.env.local` (the production env pull had replaced my local entry), so the composed shell never rendered here; (2) the composer seeded `agency_business_identity.public_name` once (the first acceptance compose was case C08 "Aura Models") and never updated it, and the legacy header + chat launcher read that row. The composer now updates `public_name` from the brief's stated name, busts every tenant cache surface after a compose, and cinematic Looks float their own header over the hero (`*-hero-shell-1440/390.jpg`). The "Rendering…" pill and the "Signed in to …" bar are dev overlays.
