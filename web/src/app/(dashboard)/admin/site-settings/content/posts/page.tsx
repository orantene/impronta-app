...
export default async function CmsPostsListPage() {
  ...
  if (error) {
    logServerError("admin/cms-posts/list", error);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className={ADMIN_SECTION_TITLE_CLASS}>Posts</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Editorial posts at <span className="font-mono">/posts/…</span> (EN) and{" "}
            <span className="font-mono">/es/posts/…</span> (ES).
          </p>
        </div>
        <Button asChild>
          <Link href="/admin/site-settings/content/posts/new">New post</Link>
        </Button>
      </div>

      <DashboardSectionCard title="All posts" description="Draft, published, and archived." titleClassName={ADMIN_SECTION_TITLE_CLASS}>
        {rows.length === 0 ? (
          <EmptyState
            icon={Newspaper}
            title="No posts yet"
            description="Create a post for announcements or editorial content."
          >
            <Button asChild>
              <Link href="/admin/site-settings/content/posts/new">New post</Link>
            </Button>
