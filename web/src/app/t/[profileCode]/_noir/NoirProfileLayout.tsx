...
[data-profile-theme="noir"] .nf-bio--auto{ font-style:italic; color:rgba(236,228,211,0.55); letter-spacing:0.01em; }
...
/* digitals */
[data-profile-theme="noir"] .nf-digitals{ display:grid; grid-template-columns:repeat(auto-fit, minmax(150px, 220px)); gap:1px; background:var(--nf-line); border:1px solid var(--nf-line); width:fit-content; max-width:100%; }
...
export function NoirProfileLayout(props: LightProfileLayoutProps) {
  ...
  const labels = {
    ...
    clientsEyebrow: pickLocale(locale, { en: "Specialties", es: "Especialidades" }),
    ...
    >
      ...
      <section className="nf-wrap nf-hero" data-profile-section="hero">
        ...
        <div className="nf-intro">
          ...
          {aboutText.trim() ? (
            <p className="nf-bio">{aboutText}</p>
          ) : primaryType || livesIn || agency ? (
            // Empty-bio fallback: a derived, muted descriptor so the hero
            // never opens on a blank void (most free-plan profiles have no
            // bio yet). Locale-safe — just the facts, joined.
            <p className="nf-bio nf-bio--auto">
              {[primaryType, livesIn, agency].filter(Boolean).join(" · ")}
            </p>
          ) : null}
