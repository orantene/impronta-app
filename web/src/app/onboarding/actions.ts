...
export async function completeTalentLocationOnboarding(
  ...
  if (user.email) {
    sendEmail({
      to: user.email,
      subject: `Welcome to ${PLATFORM_BRAND.name} — your profile is ready`,
      html: renderTalentWelcomeEmail({ displayName: display_name }),
    }).catch(() => {});
  }
...
function renderTalentWelcomeEmail({ displayName }: { displayName: string }): string {
...
<html><body style="margin:0;padding:32px 16px;background:#f1ede3;font-family:'Geist',Inter,system-ui,sans-serif;color:#0f1714;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#fffdf7;border-radius:20px;border:1px solid rgba(15,23,20,0.08);">
    <tr><td style="padding:40px 40px 32px;">
      <div style="font-size:11px;font-weight:600;letter-spacing:0.26em;text-transform:uppercase;color:#1f4a3a;">Profile created</div>
      <h1 style="font-family:'Geist',Inter,system-ui,sans-serif;font-size:28px;line-height:1.1;font-weight:500;margin:16px 0 0;color:#0f1714;letter-spacing:-0.025em;">You&rsquo;re on ${escapeHtml(PLATFORM_BRAND.name)}, ${escapeHtml(displayName)}.</h1>
      <p style="margin:20px 0 0;color:#3a4541;font-size:15px;line-height:1.6;">Your talent profile has been created. It&rsquo;s currently set to draft — an agency admin can make it visible once you&rsquo;re added to their roster.</p>
      <div style="margin:28px 0 0;padding:20px;border-radius:16px;background:#f4f2e8;border:1px solid rgba(15,23,20,0.08);">
        <p style="margin:0;color:#3a4541;font-size:14px;line-height:1.6;">Next: complete your profile — add a photo, your category, measurements, and availability. The more complete it is, the faster bookings come in.</p>
        <p style="margin:18px 0 0;">
          <a href="${appUrl}/talent/profile" style="display:inline-block;padding:12px 18px;border-radius:999px;background:#1f4a3a;color:#fffdf7;font-size:14px;font-weight:600;text-decoration:none;">Open your profile</a>
        </p>
      </div>
      <hr style="border:none;border-top:1px solid rgba(15,23,20,0.08);margin:32px 0;"/>
      <p style="margin:0;color:#6b766f;font-size:13px;line-height:1.6;">— The ${escapeHtml(PLATFORM_BRAND.name)} team<br/>${escapeHtml(PLATFORM_BRAND.stage)} · ${new Date().getFullYear()}</p>
    </td></tr>
  </table>
</body></html>`;
}
