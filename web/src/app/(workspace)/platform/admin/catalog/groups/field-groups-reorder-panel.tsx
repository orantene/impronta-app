...
function moveItem(items: ReorderGroup[], fromId: string, toId: string): ReorderGroup[] {
...
export function FieldGroupsReorderPanel({ groups }: { groups: ReorderGroup[] }) {
  ...
  return (
    <form action={reorderPlatformFieldGroupsAction} style={{ display: "grid", gap: 10 }}>
        ...
        {items.map((group, index) => (
          <div
            ...
            style={{
              ...
              gap: 10,
              padding: "9px 10px",
              border: `1px solid ${draggingId === group.id ? "rgba(93,211,160,0.45)" : colors.border}`,
              borderRadius: 10,
              background: draggingId === group.id ? "rgba(93,211,160,0.10)" : colors.card,
              color: colors.ink,
              cursor: "grab",
          ...
          >
            ...
            <span style={{ minWidth: 0 }}>
              <span style={{ fontWeight: 750, fontSize: 12.5 }}>{group.name_en}</span>
              <span
                style={{
                  display: "block",
                  overflow: "hidden",
