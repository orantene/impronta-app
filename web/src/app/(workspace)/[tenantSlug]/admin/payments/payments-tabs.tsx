        ...
        {!board.takings.ok ? (
          ...
          <div className="rounded-[14px] border border-admin-border bg-admin-card">
            <table className="w-full border-collapse">
              <thead>
                <tr className="text-left text-admin-11 font-semibold uppercase leading-[1.2] tracking-[0.05em] text-admin-ink-muted">
                  <th className="px-[16px] py-[10px] font-semibold">{t("colMethod")}</th>
                  <th className="px-[16px] py-[10px] text-right font-semibold">{t("colCount")}</th>
                  <th className="px-[16px] py-[10px] text-right font-semibold">{t("colAmount")}</th>
                </tr>
              </thead>
              <tbody>
                {board.takings.groups.map((g) => {
                  ...
                  return (
                    <tr key={`${g.currency}:${g.method}`} data-method={g.method} className="border-t border-admin-border-soft text-admin-12h leading-[1.2]">
        ...
        {!board.owed.ok ? (
          ...
          <div className="flex flex-wrap items-center gap-[24px] rounded-[14px] border border-admin-border bg-admin-card px-[20px] py-[16px]">
            {board.owed.totals.map((c) => (
              <span key={c.currency} className="text-[24px] font-semibold leading-[1.2] tabular-nums text-admin-ink" data-owed-currency={c.currency}>
...
export function RefundsTab({ board, t, tenantSlug, locale }: { board: PaymentsBoard; t: T; tenantSlug: string; locale: string }) {
  ...
  return (
    <Section title={t("refundsTitle")} sub={t("refundsSub")} testId="payments-refunds" quiet>
      {!board.refunds.ok ? (
            ...
            <div className="rounded-[14px] border border-admin-border bg-admin-card" data-testid="payments-refunds-pending">
              ...
              {pending.map((r) => (
                <div key={r.key} className={`${GRID_ROW} ${cols}`} data-refund-pending>
                  ...
                  <span className="truncate text-admin-ink-muted" title={r.detail}>{r.detail}</span>
                  <StatePill block tone="coral" state="pending">{t("refundPending")}</StatePill>
                </div>
              ))}
            </div>
          ) : null}
...
export function AttemptsTab({ board, t, locale, now }: { board: PaymentsBoard; t: T; locale: string; now: Date }) {
  const cols = "grid-cols-[90px_110px_1.4fr_1.2fr_190px_110px]";
  return (
    <Section title={t("attemptsTitle")} sub={t("attemptsSub")} testId="payments-attempts" quiet>
      {!board.exceptions.ok ? (
          ...
          {board.exceptions.unknownAttempts.map((r) => {
            ...
            return (
              <div key={r.key} className={`${GRID_ROW} ${cols}`} data-attempt-row>
                ...
                <StatePill block tone="critical" state="unknown">
...
export function DrawersTab({ board, t, locale, currency }: { board: PaymentsBoard; t: T; locale: string; currency: string }) {
  ...
  return (
    <Section title={t("drawerTitle")} sub={t("drawerSub")} testId="payments-drawers" quiet>
      {!board.drawers.ok ? (
        ...
        <div className="rounded-[14px] border border-admin-border bg-admin-card">
          ...
          {board.drawers.rows.map((s) => (
            <div key={s.id} className={`${GRID_ROW} ${cols}`} data-drawer-row data-drawer-status={s.status}>
              ...
              {s.status === "open" ? (
                <StatePill block tone="indigo" state="open">{t("drawerOpenLabel")}</StatePill>
              ) : (
                <StatePill block tone="neutral" state="closed">{t("drawerClosedLabel")}</StatePill>
              )}
            </div>
          ))}
        </div>
...
export function PayoutsTab({ board, t, tenantSlug }: { board: PaymentsBoard; t: T; tenantSlug: string }) {
  return (
    <Section title={t("payoutsTitle")} sub={t("payoutsSub")} testId="payments-payouts" quiet>
      {!board.payout.ok ? (
...
export function ReconciliationTab({ board, t }: { board: PaymentsBoard; t: T }) {
  ...
  return (
    <Section title={t("reconciliationTitle")} sub={t("reconciliationSub")} testId="payments-reconciliation" quiet>
      <div className="rounded-[14px] border border-admin-border bg-admin-card px-[20px] py-[16px]">
