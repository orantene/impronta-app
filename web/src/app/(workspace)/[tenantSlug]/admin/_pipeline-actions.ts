...
// ─── Coordinator management ────────────────────────────────────────────
// Originally landed in commit deb5abe6 (A5 fix). Lost in a subsequent
// rewrite of this file; re-applied here to keep ReassignCoordinatorSheet
// wired end-to-end. See `internal/messages.tsx` for the consumer.
...
export async function loadWorkspaceCoordinatorCandidates(
  ...
  try {
    ...
    // Load active-inquiry count per candidate so the picker can show
    // load alongside the name. One query for all candidates.
    const userIds = filtered.map((r) => r.profile_id);
    const loadCounts: Record<string, number> = {};
    if (userIds.length > 0) {
      const { data: loadRows } = await supabase
    ...
    // Sort: active first, then by current load ascending (least busy first).
    candidates.sort((a, b) => {
      if (a.status !== b.status) return a.status === "active" ? -1 : 1;
      return a.activeInquiryCount - b.activeInquiryCount;
    });
...
/**
 * Reassign the primary coordinator on an inquiry to a new workspace
 * member. Wraps `assignCoordinator` engine call — atomically updates
 * `inquiries.coordinator_id` AND deletes the old coordinator participant
 * + inserts the new one as `status='invited'`.
 */
export async function reassignCoordinatorAction(
  _tenantSlug: string,
  inquiryId: string,
