...
type Ctx = {
  ...
  bridgeTalentPayoutSnapshot:
    ...
    | null;
  /** The talent's payout legs that did not land (reversed / failed / held). */
  ...
  const setPage = useCallback((p: WorkspacePage) => {
    ...
    if (slug) {
      ...
      if (typeof window !== "undefined" && window.location.pathname !== targetHref) {
        if (railMovesWithPushState(window.location.pathname, base, segment)) {
          // The page body is already switched above and neither the route
          // being left nor the target has server content (D-167), so the
