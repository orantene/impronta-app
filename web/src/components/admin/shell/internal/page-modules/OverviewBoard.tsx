...
export function OverviewBoard() {
  ...
  return (
    <div data-tulala-overview-board className="flex flex-col gap-[20px] font-admin-body max-[720px]:gap-[12px]">
      {/* Greeting + actions. MW02: the phone keeps the greeting and the
          subline; Open POS lives in the More sheet and the calendar is a tab. */}
      <div className="flex items-center justify-between gap-[16px]">
        <div className="min-w-0">
          <h1 className="m-0 text-[22px]! font-semibold leading-[1.15] tracking-[-0.02em] text-admin-ink">
            {greeting}
          </h1>
          <div className="mt-[4px] text-admin-13 text-admin-ink-muted max-[720px]:text-admin-12h">{subline}</div>
        </div>
      ...
      <div className="grid grid-cols-5 gap-[12px] max-[720px]:grid-cols-3 max-[720px]:gap-[8px]">
        <Kpi
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[1.35fr_1fr] gap-[16px] max-[720px]:grid-cols-1 max-[720px]:gap-[12px]">
        ...
        <section className={`${CARD} flex flex-col overflow-hidden`} aria-labelledby="tulala-needs-you">
          <div className="flex items-center justify-between gap-[10px] px-[18px] pb-[6px] pt-[16px] max-[720px]:hidden">
          ...
          {!snapshot ? (
            ...
            <ul className="m-0 flex list-none flex-col p-0">
              {snapshot.needsYou.map((row, index) => (
                <li key={row.key} className={`flex items-center gap-[14px] border-t border-admin-border-soft px-[18px] py-[12px] max-[720px]:gap-[10px] max-[720px]:px-[14px] max-[720px]:first:border-t-0 ${index >= MOBILE_QUEUE_ROWS ? "max-[720px]:hidden" : ""}`}>
        ...
        <div className="flex min-h-0 flex-col gap-[16px] max-[720px]:gap-[12px]">
