 ...
 *   - When `initialBridgeData?.roster` is non-null, the workspace roster
 *     surface renders real data.
 *   - When `initialBridgeData` is null/undefined (the default — i.e. URL
 *     does not carry `?dataSource=live`), every surface renders mock
 *     data exactly as it did before the bridge existed.
 *
 * Architecture (5-file split, post-Phase-1 bridge):
 *   page.tsx          — server-component entry. Reads `searchParams.dataSource`.
...
/**
 * Client shell for the admin-shell prototype.
 *
...
// CANONICAL_ROUTE_MATCHERS + pathIsCanonical live in ./canonical-routes.ts so
