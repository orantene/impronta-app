...
/** Floor · Timeline · List, the same segmented control wherever it sits. */
export function FloorViewSwitch({
  view,
  copy,
  onViewChange,
}: {
  ...
  return (
    <div role="tablist" className={POS_SEGMENT_TRACK}>
      {viewButtons.map(([id, label]) => (
        <button key={id} type="button" role="tab" aria-selected={view === id} data-floor-view={id} onClick={() => onViewChange(id)} className={cn(POS_SEGMENT, "px-4", view === id ? POS_SEGMENT_ACTIVE : POS_SEGMENT_IDLE)}>
          {label}
        </button>
      ))}
    </div>
  ...
  const board = (
    <div data-floor-board className={cn("relative flex min-h-0 flex-1", props.className)}>
      ...
      <div ref={floorRef} className="relative flex min-h-0 min-w-0 flex-1 flex-col gap-2.5 bg-admin-surface px-[22px] pb-3 pt-3">
        ...
        {body}
        {/* The map carries Walk-in · New reservation · Pause online bookings
            under itself (`POSLiveFloor`); the timeline and the list do not. */}
        {!props.header && view === "floor" && actionsBar}
