...
/**
 * WalkInSheet — T07. WaitingSheet — T08 over `party_waitlist`.
 */
...
import { fits, isSeatable, tableCode } from "./floor-model";
import type { FloorWaitlistEntry } from "./floor-types";
...
export type WalkInSheetProps = {
  ...
  readonly onAddToWaitlist: (input: { holderName: string; partySize: number; holderPhone?: string }) => void;
};
        ...
        <section className="flex flex-col gap-2.5">
          ...
          <div className="flex items-center gap-3 rounded-[14px] border-[1.5px] border-admin-border bg-admin-card px-4 py-3.5">
            ...
            <button
              ...
              disabled={busy}
              onClick={() => props.onAddToWaitlist({ holderName: name.trim(), partySize: party, holderPhone: phone.trim() || undefined })}
            >
              {w.addToWaitlist}
            </button>
          </div>
...
export type WaitingSheetProps = {
  ...
  readonly busy: boolean;
  readonly onClose: () => void;
  readonly onSeat: (entry: FloorWaitlistEntry) => void;
  readonly onOffer: (entry: FloorWaitlistEntry) => void;
  readonly onRemove: (entry: FloorWaitlistEntry) => void;
...
export function WaitingSheet(props: WaitingSheetProps) {
  ...
  const waiting = data.partyWaitlist;
  return (
    <PosSheet
      open={props.open}
      name="waiting"
    ...
    >
      {waiting.length === 0 ? (
        ...
        <ul className="m-0 flex list-none flex-col gap-3 p-0">
          {waiting.map((entry) => (
            <li key={entry.id} data-floor-waiting={entry.id} className="flex flex-col gap-3 rounded-[14px] border-[1.5px] border-admin-border bg-admin-card px-4 py-3.5">
              <div className="flex items-start gap-3">
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[14px] text-admin-ink-muted">
                    {interpolate(w.told, {
                      time: venueHhmm(entry.joinedAtIso, data.timeZone, data.locale),
                      n: Math.max(0, Math.round((nowMs - Date.parse(entry.joinedAtIso)) / 60_000)),
                    })}
                  </span>
                </span>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" className={POS_OUTLINE_ACTION} disabled={busy || entry.status === "notified"} onClick={() => props.onOffer(entry)}>
                  {w.offerTable}
                </button>
                <button type="button" className={POS_SECONDARY_ACTION} disabled={busy} onClick={() => props.onRemove(entry)}>
                  {w.remove}
                </button>
              </div>
              <p className="m-0 text-[12.5px] text-admin-ink-muted">{entry.status === "notified" ? w.offered : w.offerReason}</p>
            </li>
          ))}
        </ul>
      )}
