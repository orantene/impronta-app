    ...
    <ol ref={listRef} className="flex flex-col gap-2">
      ...
      */}
        ...
        return (
          ...
          >
            {showIndicator ? (
              <div
                aria-hidden
                className="pointer-events-none absolute -top-1 left-0 right-0 h-0.5 rounded-full bg-[#3d4f7c]"
              />
            ) : null}
      ...
      {dragIndex !== null && dropIndex === items.length ? (
        <li data-drag-tail aria-hidden className="h-0.5 rounded-full bg-[#3d4f7c]" />
      ) : null}
    </ol>
  );
}
