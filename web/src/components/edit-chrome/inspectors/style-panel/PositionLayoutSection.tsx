 ...
 *
 * ORDER IS THE ARGUMENT (stack-first pass, 2026-08-27). These four blocks used
 * to read Position, Transform, Layout in parent, Layout (children): the free
 * transforms sat ABOVE the two blocks that decide how children are actually
 * arranged, at the same visual weight, which is the panel telling an operator
 * that nudging a box by 8px and choosing its flow are the same kind of
 * decision. They are not. The fundamentals now come first and Transform is
 * last, carrying one line that says what it is: paint, applied after the
 * layout has already run. Every field is still here and still collapsed; this
 * is emphasis, not removal.
 */
