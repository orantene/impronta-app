...
/**
 * The storefront half of the editor mount. `edit-chrome-mount.tsx` (the
 * root-layout gate) reaches this module through a runtime `import()` only
 * once the request is a storefront path, so the builder's server graph (the
 * adapters, the composition actions, the sections registry with every
 * Editor and Component, the builder-node islands) is not part of the root
 * layout's static module graph on /admin, /login, /offline and the rest.
 * The gate already proved the path is a storefront path; the headers are
 * re-read here because the ownership resolver below wants the raw pathname
 * and the public path prefix, not the normalised form.
 */
export async function EditChromeStorefrontMount() {
