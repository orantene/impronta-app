...
export type ItemsActions = {
  ...
  readonly addLine: (input: { orderId: string; offeringId: string; units: number; sessionId?: string | null; variantId?: string | null; addonIds?: readonly string[]; expectedVersion?: number; inquiryId?: string }) => Promise<{ ok: true } | Refused>;
  ...
  for (const call of calls) {
    switch (call.action) {
      ...
      case "add_line": {
        ...
        const r = await actions.addLine({ orderId, offeringId: call.offeringId, units: call.units, sessionId: call.sessionId ?? null, variantId: call.variantId ?? null, addonIds: call.addonIds, expectedVersion: orderVersion, inquiryId: ctx.inquiryId });
        if (!r.ok) return r;
