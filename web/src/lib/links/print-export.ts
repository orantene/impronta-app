...
export async function toPrintPdfDesign(
  ...
  const bgImage = design.backgroundImage ? await pdf.embedPng(design.backgroundImage.png) : null;
  ...
  for (const { item, matrix } of encoded) {
    ...
    // A full-bleed photograph, under everything and over the fill. "cover"
    // crops to fill rather than letterboxing: a background that does not reach
    // the bleed edge produces a white hairline at the trim, which is the exact
    // defect bleed exists to prevent.
    if (bgImage && design.backgroundImage) {
      if ((design.backgroundImage.fit ?? "cover") === "stretch") {
        page.drawImage(bgImage, { x: 0, y: 0, width: pageW, height: pageH });
      } else {
        const scale = Math.max(pageW / bgImage.width, pageH / bgImage.height);
        const w = bgImage.width * scale;
        const h = bgImage.height * scale;
        page.drawImage(bgImage, { x: (pageW - w) / 2, y: (pageH - h) / 2, width: w, height: h });
      }
    }
