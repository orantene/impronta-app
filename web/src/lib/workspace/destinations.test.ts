...
test("a business workspace sees People, but no talent sub-views and no Pitches", () => {
  ...
  // The landing view and the three hat tabs (W27). The other three open the
  // roster's representation queues, whose routes call `assertRosterWorkspace`
  // and 404 for a business workspace — the nav must not offer a link into that.
  const subs = visibleSubViews(DESTINATIONS.people, business).map((s) => s.id);
  assert.deepEqual(subs, ["everyone", "talent", "bookable", "access"]);
