...
import { AGENCY_API_PREFIXES, AGENCY_STOREFRONT_PREFIXES, APP_API_EXACT_PATHS, APP_API_PREFIXES, APP_WORKSPACE_PREFIXES, AUTH_PREFIXES, CANONICAL_EVENTS_ES_PREFIX, CANONICAL_EVENTS_PREFIX, CANONICAL_GUEST_THREAD_PREFIX, CANONICAL_LINK_PREFIX, CANONICAL_MANAGE_PREFIX, CANONICAL_PAY_PREFIX, CANONICAL_RECEIPT_PREFIX, CANONICAL_TALENT_PREFIX, CANONICAL_TICKET_API_PREFIX, CANONICAL_TICKET_PREFIX, CANONICAL_VISIT_PREFIX, CHECKOUT_PREFIX, COMPLIANCE_PREFIXES, EMBED_EXACT_PATHS, EMBED_PREFIX, MARKETING_API_PREFIXES, MARKETING_PAGE_PREFIXES, PROTOTYPE_PREFIX, PWA_PATHS, SHARED_API_PREFIXES, STATIC_PATHS, WELL_KNOWN_PREFIX } from "./path-groups";
...
export function isPathAllowedForHostKind(
  ...
  if (hasPrefix(pathname, CANONICAL_TICKET_API_PREFIX)) return kind === "agency" || kind === "hub";
  ...
  if (hasPrefix(pathname, CANONICAL_EVENTS_PREFIX)) return kind === "agency" || kind === "hub";
  if (hasPrefix(pathname, CANONICAL_EVENTS_ES_PREFIX)) return kind === "agency" || kind === "hub";
  if (hasPrefix(pathname, CANONICAL_RECEIPT_PREFIX)) return kind === "agency" || kind === "hub";
  if (hasPrefix(pathname, CANONICAL_PAY_PREFIX)) return kind === "agency" || kind === "hub";
  if (hasPrefix(pathname, CANONICAL_MANAGE_PREFIX)) return kind === "agency" || kind === "hub";
