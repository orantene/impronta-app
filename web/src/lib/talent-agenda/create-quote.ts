/**
 * G2.2 / Stage B1 — Talent event/project quote drafts on the platform hub.
...
/**
 * T7.2 Event quote — draft inquiry + draft offer on hub. Optional date hold.
...
export async function createOwnEventQuote(input: {
  ...
  const { data: inquiry, error: inqErr } = await admin
    ...
    .insert({
      ...
      contact_name: "Draft (no client)",
      contact_email: `quote-draft+${actor.userId.slice(0, 8)}@tulala.local`,
      message: what,
...
/**
 * T7.3 Project quote — draft commercial booking on hub (no calendar appointment)
