...
describe("G2.2 quote writers", () => {
  it("create-quote writes inquiry_offers and booking_deliverables on hub", () => {
    const src = blankComments(readFileSync(path.join(ROOT, "create-quote.ts"), "utf8"));
  ...
  it("cancel-actions use talent ownership, not staff()", () => {
    const src = blankComments(readFileSync(path.join(ROOT, "cancel-actions.ts"), "utf8"));
