 ...
 *
 * These tests encode the CONTEXT'S shape — line totals with `units = 1` — and
 * demonstrate what the old shape produced, so the fix cannot be quietly undone.
 */
...
describe("line-item grain — the context passes TOTALS with units = 1", () => {
  ...
  test("the client subtotal matches total_price, not unit_price x units", () => {
