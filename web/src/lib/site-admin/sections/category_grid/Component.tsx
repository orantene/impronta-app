...
function ctaJustifyContent(align?: "left" | "center" | "right"): CSSProperties["justifyContent"] {
...
export function CategoryGridComponent({
  ...
  const footerCtaNode = nodePresentation?.footerCta;
  const responsiveCss = buildNodePresentationResponsiveCss({
    ...
    rules: [
      ...
      {
        selector: ".site-category-grid__head-cta",
        tablet: toCssDecls({
          justifyContent: ctaJustifyContent(footerCtaNode?.breakpoints?.tablet?.align),
        }),
        mobile: toCssDecls({
          justifyContent: ctaJustifyContent(footerCtaNode?.breakpoints?.mobile?.align),
        }),
      },
      {
        selector: ".site-category-grid__head-cta .site-prim-cta",
        tablet: ctaDecls(footerCtaNode?.breakpoints?.tablet),
        mobile: ctaDecls(footerCtaNode?.breakpoints?.mobile),
      },
    ],
  });
  ...
  return (
    ...
    >
      ...
      <Container width="standard">
        <SectionHead
          align={(headlineNode?.align ?? eyebrowNode?.align ?? copyNode?.align) === "left" ? "start" : "center"}
          eyebrow={
            eyebrow ? (
              <span
                style={{
        ...
        {footerCta ? (
          <div
            className="site-category-grid__head-cta"
            style={{
              justifyContent: ctaJustifyContent(footerCtaNode?.align),
            }}
          >
            <Cta
              href={footerCta.href}
              variant="secondary"
              builderNodeId={nodeIdsByRole?.footerCta}
              style={{
                ...buttonSize(footerCtaNode?.size),
                marginTop:
                  typeof footerCtaNode?.marginTopPx === "number"
                    ? `${footerCtaNode.marginTopPx}px`
                    : undefined,
                marginBottom:
                  typeof footerCtaNode?.marginBottomPx === "number"
                    ? `${footerCtaNode.marginBottomPx}px`
                    : undefined,
                marginInline:
                  typeof footerCtaNode?.marginLeftPx !== "number" &&
                  typeof footerCtaNode?.marginRightPx !== "number" &&
                  typeof footerCtaNode?.marginInlinePx === "number"
                    ? `${footerCtaNode.marginInlinePx}px`
                    : undefined,
                marginLeft:
                  typeof footerCtaNode?.marginLeftPx === "number"
                    ? `${footerCtaNode.marginLeftPx}px`
                    : undefined,
                marginRight:
                  typeof footerCtaNode?.marginRightPx === "number"
                    ? `${footerCtaNode.marginRightPx}px`
                    : undefined,
                paddingTop:
                  typeof footerCtaNode?.paddingTopPx === "number"
                    ? `${footerCtaNode.paddingTopPx}px`
                    : undefined,
                paddingBottom:
                  typeof footerCtaNode?.paddingBottomPx === "number"
                    ? `${footerCtaNode.paddingBottomPx}px`
                    : undefined,
                paddingInline:
                  typeof footerCtaNode?.paddingLeftPx !== "number" &&
                  typeof footerCtaNode?.paddingRightPx !== "number" &&
                  typeof footerCtaNode?.paddingInlinePx === "number"
                    ? `${footerCtaNode.paddingInlinePx}px`
                    : undefined,
                paddingLeft:
                  typeof footerCtaNode?.paddingLeftPx === "number"
                    ? `${footerCtaNode.paddingLeftPx}px`
                    : undefined,
                paddingRight:
                  typeof footerCtaNode?.paddingRightPx === "number"
                    ? `${footerCtaNode.paddingRightPx}px`
                    : undefined,
                display: visibilityDisplay(footerCtaNode?.visibility),
              }}
            >
              {footerCta.label}
            </Cta>
