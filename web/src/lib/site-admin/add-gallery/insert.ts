...
/** The add-flow result shape (mirrors `AddGalleryInsertDeps` returns). */
export type DirectoryAddCapResult = { ok: true } | { ok: false; error: string };

/**
 * Count the directory `section_embed` nodes inside a freeform `blocks` tree.
 * Tolerant of the loosely-typed JSON shape persisted to `cms_pages.blocks`
 * (an array of `BuilderNode`, but it arrives here as `unknown`).
 */
function countDirectoryEmbedsInBlocks(blocks: unknown): number {
  let count = 0;
  const visit = (node: unknown): void => {
    if (!node || typeof node !== "object") return;
    const n = node as {
      kind?: unknown;
      props?: { sectionTypeKey?: unknown } | null;
      children?: unknown;
    };
    if (
      n.kind === "section_embed" &&
      n.props != null &&
      n.props.sectionTypeKey === DIRECTORY_SECTION_TYPE_KEY
    ) {
      count += 1;
    }
  ...
  if (Array.isArray(blocks)) {
    for (const root of blocks) visit(root);
  }
  return count;
}

/**
 * Add-path gate for directory sections. Call this BEFORE performing the insert
 * (e.g. ahead of `performAddGalleryInsert`) when the item targets the directory
 * section; if it returns `{ ok: false }`, surface `error` and skip the insert.
 *
 * Behaviour:
 *   - Non-directory items: always `{ ok: true }` (no other section type is
 *     gated by this cap).
 *   - Directory items: loads the tenant's `agencies.plan_tier` + the live count
 *     of existing directory `section_embed` instances across the tenant's
 *     freeform `cms_pages`, then calls `evaluateDirectoryInstanceAdd`
 *     (Free=0 dedicated / Studio=1 / Agency+=unlimited). When the cap is hit it
 *     returns the helper's plain-language `reason` as the add-flow error.
 *
 * Server-only by construction (it reads Supabase), but this module is imported
 * by client components too (preview modal, lab stage, drag), so the Supabase +
 * auth + plan helpers are pulled in via dynamic `import()` INSIDE the function.
 * That keeps the synchronous, pure exports above free of any `server-only` /
 * DOM-poisoning transitive imports.
 *
 * Fail-open: any auth/scope/query failure resolves to `{ ok: true }`. This cap
 * is a soft product gate on a count; the caps that protect data integrity
 * (seat / roster / public-display) fail CLOSED in their own layers, so a
 * best-effort COUNT here is the right tradeoff over bricking a legitimate add.
 */
export async function evaluateDirectoryAddCapForTenant(
  item: Pick<AddGalleryItem, "insertMethod" | "sectionEmbedKey">,
): Promise<DirectoryAddCapResult> {
  if (!addGalleryItemTargetsDirectory(item)) return { ok: true };

  try {
    const [{ requireStaff }, { requireTenantScope }, { evaluateDirectoryInstanceAdd }] =
      await Promise.all([
        import("@/lib/server/action-guards"),
        import("@/lib/saas/scope"),
        import("@/lib/site-admin/sections/directory/meta"),
      ]);
    ...
    const { data: pages, error: pagesError } = await auth.supabase
      ...
      .eq("is_freeform", true)
      .neq("status", "archived")
      .returns<Array<{ blocks: unknown }>>();
    // Fail-open on a count error: never block a legitimate add on a flaky read.
    if (pagesError) return { ok: true };

    const current = (pages ?? []).reduce(
      (sum, page) => sum + countDirectoryEmbedsInBlocks(page.blocks),
      0,
    );

    const verdict = evaluateDirectoryInstanceAdd({
      planTier: agency?.plan_tier ?? null,
      current,
    });
    if (verdict.canAdd) return { ok: true };
    return {
      ok: false,
      error:
        verdict.reason ??
        "Your plan's directory limit has been reached. Upgrade to add another directory page.",
    };
  } catch {
    return { ok: true };
  }
