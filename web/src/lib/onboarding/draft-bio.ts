...
export function draftBio(facts: BioFacts, locale: "en" | "es"): string {
  ...
  const parts: string[] = [];
  if (locale === "es") {
    parts.push(what ? (facts.name ? `Soy ${facts.name}, ${lower(what)}` : `${cap(what)}`) : facts.name ? `Soy ${facts.name}` : "Trabajo por mi cuenta");
    ...
    parts.push(what ? (facts.name ? `I'm ${facts.name}, a ${lower(what)}` : `${cap(what)}`) : facts.name ? `I'm ${facts.name}` : "I work independently");
