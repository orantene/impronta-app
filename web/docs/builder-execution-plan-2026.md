...
| Date | Change |
|------|--------|
| 2026-05-10 | **P7A-3 (partial, code):** Centralized gap-index → `moveBuilderNodeToParentIndex` sibling index in [`builder-node-sibling-drop.ts`](../src/components/edit-chrome/builder-node-sibling-drop.ts); navigator child drag, canvas nested-blocks panel, and inspector child list share [`siblingDropGapToMoveIndex`](../src/components/edit-chrome/builder-node-sibling-drop.ts) + [`builder-node-sibling-drop.test.ts`](../src/components/edit-chrome/builder-node-sibling-drop.test.ts). **Full P7A-3 acceptance** still needs manual **navigator vs canvas** reorder QA on a registered host. |
