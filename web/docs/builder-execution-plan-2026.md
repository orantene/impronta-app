...
| 2026-05-13 | **Navigator e2e hygiene:** [`smoke.spec.ts`](../e2e/smoke.spec.ts) — **layers collapse + search** drops brittle per-row DOM count after collapse when selection keeps layers open; **child reorder** renamed to action-button-only path (move down + targeted move up; no synthetic drag). Added [`test:e2e:impronta-navigator-child-reorder`](../package.json) + [`test:e2e:impronta-navigator-layers-collapse-search`](../package.json). |
