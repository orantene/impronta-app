...
| **7** | Partial | **7A code:** allow-list kinds, draft/publish tree paths, nested insert governance, Free draft save + publish preflight, Card / CTA group kinds ([§7A MVP](#7a-mvp-element-allow-list)). **7A product:** not claimed shipped until **7A Reality Test** + evidence on a registered host. **7B:** bounded **Hero layout pilot** (`centered` / `split-left` / `split-right` on legacy Hero row) shipped on `phase-1` — does **not** replace 7A. **P7B-2+ / 7C** remain after 7A acceptance. |
...
| 5 | **`exec-p7a-2-selection` / `p7a-2-multi-select`** | Honest selection | [`edit-context.tsx`](../src/components/edit-chrome/edit-context.tsx) derives `selectedBuilderNodeId` via **`useMemo` + [`resolveHonestSelectedBuilderNodeId`](../src/components/edit-chrome/inspectors/builder-node-content-utils.ts)** (no ghost override vs tree / section maps); `selectBuilderNode` still rejects unknown ids; **inspector** `selectionTreeMismatch` → skeleton remains defense in depth | Registered-host QA |
...
| 7 | **`exec-p7a-4-roundtrip` / `p7a-4-cache`** | Publish + reopen truth | Automated slices in `p7a-reorder-publish-parity.test.ts` (nested reorder, **`blank_section`**, **cross-section body↔body**, **cross-slot body→footer**); human **7A Reality Test** still authoritative | Evidence doc |
...
| 10 | **`pr-p6-2`** | `tagFor` / revalidate audit | **`publishPageSnapshot`** ([`page-composer-action.ts`](../src/lib/site-admin/edit-mode/page-composer-action.ts)) revalidates **`pages:{pageId}`** + **`pages-all`** on every CMS publish, and adds **`storefront`** when `system_template_key === "site_shell"`; **`site-shell-backfill-action.ts`** seed publish pairs **`pages-all` + `storefront`** to match homepage publish + `republishSiteShellSnapshot` (`composition-actions.ts`). Cross-check vs [phase-b-site-shell.md](./phase-b-site-shell.md) + [`cache-tags.ts`](../src/lib/site-admin/cache-tags.ts). | Smoke + tenant isolation when touched |

**Park until 7A accepted (expansion only):** full **`pr-p7b-*` / `p7b-var-*` inner-tree** work, all **`7c-*`**, all **`pv1-*`**, `shell-no-fake-model`, `strat-*` (unless copy blocks a ship). **Exception:** bounded **Hero layout pilot** (field-backed variants on the existing Hero section) may ship in parallel; it does **not** close 7A or Phase 0 matrix.
...
| Date | Change |
|------|--------|
| 2026-05-15 | **Plan / batch alignment:** [`builder-execution-batches.md`](./builder-execution-batches.md) — Batch 03 adds **promote + registered-host re-verify** path (see [`OPERATING.md`](../../OPERATING.md) §3, [`CLAUDE.md`](../../CLAUDE.md)); localhost evidence [`qa-evidence/phase0-localhost-2026-05-13.md`](./qa-evidence/phase0-localhost-2026-05-13.md). Batch 06 reconciled with **P7B Hero layout pilot** on `phase-1` vs **parked** 7C / deep P7B. **Hub IA:** `/admin/site` thin redirect + dashboard nav alignment (`web/src/app/admin/site/page.tsx`, architecture + command palette + topbar hub fallback). **BUG-005:** publish drawer visible zero-diff copy + **Reload composition**; `prefer-const` + `cancel-flow` apostrophe lint fixes. |
| 2026-05-14 | **BUG-005 (publish drawer SR):** [`publish-drawer.tsx`](../src/components/edit-chrome/publish-drawer.tsx) — when **changes since last publish = 0**, a polite **`sr-only`** status reminds that **canvas/preview can still disagree** with the diff; verify Preview / preflight before publishing. |
...
| 2026-05-14 | **BUG-004 (preflight slice — long CDN URLs):** [`publish-preflight-layout-rules.ts`](../src/lib/site-admin/edit-mode/publish-preflight-layout-rules.ts) — layout overflow scan skips **whole-field `https?://` and `//` URLs** and path leaves **`src` / `poster`** so masonry/gallery props do not false-block publish; tests in [`publish-preflight-layout-rules.test.ts`](../src/lib/site-admin/edit-mode/publish-preflight-layout-rules.test.ts). Polluted drafts / real long copy still warn or block. **Batch 04** operating checklist + **five-step continuation** table in [`phase-0-qa-registered-host.md`](./phase-0-qa-registered-host.md) / [`builder-execution-batches.md`](./builder-execution-batches.md). |
| 2026-05-14 | **BUG-010 / BUG-009 (mitigations):** [`topbar.tsx`](../src/components/edit-chrome/topbar.tsx) — `[data-edit-topbar]` is a horizontal scroll container (`overflow-x-auto`) with an inner `min-w-max` row so **Publish** / **Exit** are reachable on narrow viewports; **Live · published** chip hidden below `lg`. [`ClientSpeedInsights`](../src/components/analytics/client-speed-insights.tsx) + [`layout.tsx`](../src/app/layout.tsx) mount **Speed Insights** after hydration to reduce MIME / hydration noise. Local Playwright: `npm run test:e2e:impronta-topbar-publish-narrow`. **Batch 03** stays open until registered-host matrix is re-passed ([`phase-0-qa-registered-host.md`](./phase-0-qa-registered-host.md)). |
...
| 2026-05-12 | **P7A-2 (`exec-p7a-2-selection` — synchronous honest id):** [`resolveHonestSelectedBuilderNodeId`](../src/components/edit-chrome/inspectors/builder-node-content-utils.ts) + [`edit-context.tsx`](../src/components/edit-chrome/edit-context.tsx) — context `selectedBuilderNodeId` never surfaces an override **missing from `builderTree`** or **owned by another section** in the same render as tree/maps (existing `useEffect` still clears stale override state). Tests in [`builder-node-content.test.ts`](../src/components/edit-chrome/inspectors/builder-node-content.test.ts). |
