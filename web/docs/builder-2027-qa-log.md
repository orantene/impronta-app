...
## Live click-through — the four native header widgets (2026-09-03)

Run by hand in Chrome against **production** (`improntamodels.com`), on deployment
`tulala-6umw7irbb`, sha `e7e299527`, state READY. Not a test run: every row below is a
click I made and a screen I looked at. These four widgets shipped in #1502 and had
never been rendered in front of a human until now.

| # | Widget | Step | Result | Sev |
|---|---|---|---|---|
| W1 | `header_search` | Click the magnifier in the header | Routes to `/directory` with a live search input. Typed "dancer for a beach wedding" — text lands in React state, 49 profiles listed, filter rail live | PASS |
| W2 | `header_account` | Click the person icon | Menu opens with the signed-in identity: Account / super admin, Dashboard, Inquiries, Sign out | PASS |
| W3 | `header_inquiry` | Click the badge (showing 3) | Booking-assistant drawer opens: "Not sent yet", Home / Chat / Lineup / Inquiries tabs, "Send to agency". **Not submitted** — opening is the assertion, sending would create a real inquiry on a live client site | PASS |
| W4 | `header_language` | Click ES | Real content swap to `/es/directory`. Nav, hero, filter labels and talent roles all translate ("Encuentra tu elenco.", "Modelo de moda", "Grupo de Baile", "Tu lista · 3") | PASS, with F16 |

All four render `data-header-widget-mode="live"` on `/` and `/fashion-models`, and both
pages carry `role="banner"` + `role="contentinfo"`. Console is clean on a fresh ES load,
including after hydration.

**A caution about W4.** My first screenshot after clicking ES showed English and I nearly
logged a defect. The screenshot had been taken mid-navigation. Re-reading the tab showed
`/es/directory` already resolved. A screenshot is a sample, not a settled state — confirm
against the URL and the DOM before calling a swap broken.

**`featured_talent` is still unproven.** It renders empty in the fidelity harness, which is
consistent with the harness having no talent rows, but that is an explanation and not
evidence. It is not asserted anywhere until it is seen with real data.

| # | Sev | Surface | Finding | Evidence | Status |
|---|---|---|---|---|---|
| F16 | P1 | i18n / directory facets | On `/es/directory` the facet **labels** translate but the facet **VALUES** do not. "Disponibilidad", "Largo de cabello", "Color de cabello", "Industrias", "Altura" are all Spanish; the options under them read "Available Now", "Available This Week", "By Request", "Extra Long", "Dark Brown", "Light Brown". Counted on the served HTML: each of those English strings appears exactly once on the EN page and exactly once on the ES page — identical, so nothing is being translated, and this is not a fallback for a missing string. Consistent with facet values living in `talent_profile_taxonomy` rather than the i18n catalog (see reference_es_copy_surfaces — ES copy lives in four stores). Live on a client-facing page today. | `curl` diff of `/directory` vs `/es/directory`, 2026-09-03 | open — owner is Directory & Profile Engine, not Builder 2027 |
