...
| B.2.A — Shell rendering replacement on impronta | shipped | 2026-04-26 | 2026-04-26 | (this commit) | Schema extension: site_header.authArea preserves account menu / language toggle / discovery search. HeaderAuthArea reuses existing widgets. Backfill action seeds impronta with REAL identity + nav data, ALL auth flags=true. Storefront mutex: PublicHeader OR PublishedShellHeader, never both. Env flag flipped: ENABLE_SITE_SHELL=tenants + impronta in allow-list. |
| B.2.B — EditShell wiring + publish round trip + migration safeguard | shipped | 2026-04-26 | 2026-04-26 | `08e7a2a` | Shell sections wrapped with `data-cms-section` markers so the existing selection layer + inspector + save action all work universally with no special-case code. Publish action extended to also auto-publish draft shell sections + republish the shell snapshot in one click. New `prebuild` CI check (`scripts/check-migrations-applied.mjs`) fails the build when any local migration isn't applied to the linked Supabase project — prevents silent schema drift like the 8-migrations gap surfaced in B.2.A. |
| B.2.C — Lived-experience fix: inserter + Navigator | shipped | 2026-04-26 | 2026-04-26 | (this commit) | Fix #1: composition-inserter no longer emits inserter zones for shell slots (`header` / `footer`), so the top-of-canvas `+` no longer hijacks clicks meant for the shell header. Fix #2: Navigator panel now renders a "Site shell" group above "Page" listing the header + footer rows; selecting from there behaves identically to canvas click. Two surgical edits, no special shell mental model. |
