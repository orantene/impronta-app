...
| 1.9 | **NEW, live QA 2026-08-07 — P1, and it BLOCKS verifying 1.1.** The Revisions drawer on a cms page returns **"Select an agency workspace before viewing revisions"** while the operator is signed in, in edit mode, on that very workspace's page (`/w/qa-agency-244988/wave1-about`, panel titled "Revisions · Wave1 About"). `loadHomepageRevisionsAction` (`revisions-actions.ts:100`) resolves `requireTenantScope()` exactly as `composition-actions.ts` does, and the composition load on the SAME page succeeds, so the scope is resolvable and something about the revisions call path is not seeing it. Note the action name: it takes only `{locale}` and no pageId, so a cms page may be calling the homepage revisions action. CONSEQUENCE: restore is UNREACHABLE from the UI on ordinary pages, so #1018's restore fix (1.1) is correct in code and unit-tested but cannot be exercised by an operator. Fix this before claiming 1.1 done. |
...
| 2.4b | **ROOT CAUSE for 2.0 + 2.4, diagnosed 2026-08-06.** The cms-page route (`app/(public)/p/[[...slug]]/page.tsx`) renders the freeform tree SERVER-side via `renderFreeformPageRootTree` and mounts NO `ClientBuilderCanvas` — that component mounts only in the homepage path (`home/homepage-cms-sections.tsx:365`). So on ordinary pages every mutation (gallery insert, Duplicate, style commit) changes the tree but nothing repaints, and the no-canvas fallback refresh is not landing after freeform inserts on this surface. Verified live: gallery insert on a cms page persisted 9 nodes while the canvas showed 0 until a hard reload. The fix is to give the cms-page edit surface the same client canvas the homepage has (the `in-editor-canvas-region` / `BuilderEditorMount` plumbing already exists for Lab surfaces) OR make the no-canvas refresh reliable there. This is a real build, not a patch: it is THE wave-2 centerpiece and likely fixes 2.0 and 2.4 in one move. |
| 2.0 | **NEW, live QA 2026-08-06 — P1. CORRECTED DIAGNOSIS, read the whole row.** First reported as "an empty page cannot receive its first section" because clicking a gallery card left `builderNodes` at 0 with no error. That was WRONG. Reloading showed 9 nodes with real content: the insert runs, persists, and the panel closes correctly. What actually fails is the CANVAS REPAINT — the new section is invisible until a manual reload, so the operator sees a dead button and clicks again. Same class as the undo bug (#996) and the toolbar-Duplicate item 2.4: the data was always right, the canvas lied about it. Fix belongs with 2.4, they are almost certainly one root cause. LESSON, again: verify against persisted state, never against the canvas alone. |
