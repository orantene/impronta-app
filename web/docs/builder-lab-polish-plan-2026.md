...
3. **Gate before commit, every task:** from `web/`: `NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit && npm run lint && npm run verify:server-actions && npm run test:builder && npm run test:builder-chrome`. A 0-error run that OOM-crashed is **not** clean. New logic that can be unit-tested gets a test added to the `test:builder` or `test:builder-chrome` list. **`verify:server-actions` is mandatory:** tsc/lint/tsx-tests do NOT catch non-async exports in `"use server"` files, which break `next build`; this exact class shipped twice undetected and was only caught by live-runtime QA (see [`builder-lab-completion-audit-2026.md`](./builder-lab-completion-audit-2026.md)).
