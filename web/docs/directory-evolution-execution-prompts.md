...
| **B9** | Persistent shortlist + inquiry-from-shortlist | Storefront | M | 🔥 | — | ⚠️ | `976b29b0a` — _favorites+cart system shipped end-to-end_: card bookmark + INQUIRE button, two-icon header (♥/✈) globally on every public page, two right-anchored drawers, guest→client merge bridge mirrors localStorage favorites → `client_favorites` on signup. Gold "1" badge proven in Chrome QA. **Honest gap**: `/api/directory/talents-by-ids` (the drawer's batch metadata fetch) returns 404 from curl + falls into the silent-catch in browser, so the drawer shows "N hidden — may no longer be available" instead of mini-cards. Favorite IS persisted (localStorage + count badge accurate); only the rich rendering inside the drawer needs the route plumbing fixed in a follow-up. |
