...
| 11.4 | Renewal & dunning | ⬜ | 🟡 `invoice_payment_failed`→re-sync (past_due) + `customer.subscription.updated`→`syncStripeSubscriptionToDb` (status map) classified. **Hardened by PR #377:** talent lifecycle now uses `mapStripeStatus` (unpaid→past_due) so a dunning subscription.updated no longer violates the CHECK + loops. Remaining follow-up (§16.4): `invoice.payment_succeeded` should re-sync to active directly (today relies on the trailing subscription.updated). Not force-driven (hard to induce past_due in test). **2026-06-14: re-sync follow-up CONFIRMED SHIPPED (PR #383) — `webhook-routing.ts` exposes the subscription id on `invoice.payment_succeeded` so a dunning-recovery payment re-syncs to active directly; the `invoice_payment_failed`/`succeeded` handlers both `subscriptions.retrieve → syncSubscriptionByType`.** CODE COMPLETE: classification unit-tested (webhook-routing.test), sync fn proven 11.1/11.5, re-sync wired. Residual = the live past_due→recovery round-trip needs a Stripe **test-clock** dunning lifecycle (low-value verification of proven code; deferred). | code complete; live test-clock round-trip deferred |
