...
9. **Dead admin-shell nav (latent)** — the unmounted `admin-shell.tsx` + top bar + palette + peek-sheets hardcode renamed/never-built routes (`/admin/inquiries`→`/admin/work`, `/admin/users`, `/admin/bookings/new`, …). It doesn't ship (not mounted) but is a trap. **Delete the dead shell** or wire its `navPathOverrides` remap. Confirm it's truly unmounted first (grep imports).
