...
--
-- RECOVERED FILE. This migration was applied to production on 2026-08-28 by a
-- session that ended without committing it, leaving the database ahead of the
-- repo and `supabase db push` refusing to run for everyone
-- (LegacyDbPushMissingLocalError). The SQL below is recovered verbatim from
-- supabase_migrations.schema_migrations, so it is exactly what ran. Re-running
-- it is a no-op: every statement is idempotent by construction.
--
-- WHY THE ORIGINAL EXISTS
-- ───────────────
