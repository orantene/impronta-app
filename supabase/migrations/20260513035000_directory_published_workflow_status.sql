-- Widen directory visibility to include workflow_status = 'published' alongside 'approved'.
--
-- Background: the admin single-talent workflow UI can set workflow_status='published'
-- (via updateRosterTalentWorkflow in admin/roster/[id]/actions.ts), while the bulk
-- publish action sets 'approved'. The public directory query was previously gated on
-- workflow_status='approved' only, causing profiles with status='published' to be
-- invisible on the storefront.
--
-- Fix: drop and recreate the partial indexes and the search RPC to accept both values.

BEGIN;

-- Drop old partial indexes (they hard-code workflow_status = 'approved')
...
-- Recreate indexes with workflow_status IN ('approved', 'published')
...
-- Recreate search RPC with workflow_status IN ('approved', 'published')
