BEGIN;
-- ---------------------------------------------------------------------------
-- Add canonical identity fields to talent_profiles
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Replace onboarding RPC to accept full identity fields at registration
-- The old signature had 16 params (locations only). New version adds 7
-- identity params (all optional with defaults so callers can omit them).
-- ---------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.complete_talent_onboarding_with_locations(
  ...
  -- identity fields (all optional so existing tooling that omits them still works)
...
BEGIN
  ...
  -- Prefer the submitted display_name; fall back to auth metadata
