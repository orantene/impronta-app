-- details v3 §5 follow-up: inquiry-side logistics columns + RPC swap
--
-- Companion to 20260513225052_agency_bookings_details_continuity.sql, which
-- added the same columns to `agency_bookings` and left NULL placeholders in
-- `engine_convert_to_booking` until the inquiry side existed. This migration
-- adds the 7 columns to `inquiries` and recreates the convert RPC to populate
-- the booking columns from the matching inquiry columns at conversion time.
--
...
-- ---------------------------------------------------------------------------
-- 1. New columns on inquiries (mirror of agency_bookings additions).
-- ---------------------------------------------------------------------------
...
COMMENT ON COLUMN public.inquiries.wardrobe_notes  IS 'details v3 §5: wardrobe / dress code requirements; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.equipment_notes IS 'details v3 §5: equipment / tech rider; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.transport_notes IS 'details v3 §5: transport / transfer notes; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.lodging_notes   IS 'details v3 §5: lodging / accommodation notes; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.meals_notes     IS 'details v3 §5: meals / catering notes; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.access_notes    IS 'details v3 §5: venue access / entry instructions; carried to agency_bookings on conversion.';
COMMENT ON COLUMN public.inquiries.deadline_at     IS 'details v3 §5: hard deadline; carried to agency_bookings on conversion.';

-- ---------------------------------------------------------------------------
-- 2. Recreate engine_convert_to_booking to read from the new columns instead
--    of the NULL placeholders. Signature unchanged (UUID, UUID, INT, TEXT
--    DEFAULT NULL) so all existing callers continue to work.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  --
  -- Operational logistics fields now flow from the inquiry's own columns
  -- (added in this migration). `timezone` mirrors `event_timezone` so both
  -- columns stay in sync going forward.
