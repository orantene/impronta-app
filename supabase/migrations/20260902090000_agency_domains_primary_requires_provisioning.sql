-- Phase 4 — custom domains: primary host promotion now requires
-- routing + SSL readiness, not only TXT ownership verification.
...
BEGIN
  ...
  IF v_kind = 'custom'
     AND v_status NOT IN ('ssl_provisioned', 'active') THEN
...
COMMENT ON FUNCTION public.set_primary_agency_domain(UUID, TEXT) IS
  'Atomically switches the canonical public host for a tenant. Owner-only. Custom domains must finish routing / SSL readiness before they can become primary.';
