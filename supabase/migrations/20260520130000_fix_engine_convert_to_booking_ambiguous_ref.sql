-- Fix: engine_convert_to_booking had ambiguous column reference "booking_id"
-- in the booking_talent totals rollup subquery.
-- WHERE booking_id = booking_id compared column to itself (always true, wrong and ambiguous).
-- Fix: qualify the column with the table name so PL/pgSQL resolves the variable correctly.
...
BEGIN
  ...
  -- Fix: qualify bt.booking_id with table alias to avoid ambiguity with the PL/pgSQL variable
  ...
  FROM (
    SELECT
      bt.booking_id AS id,
      COALESCE(SUM(bt.talent_cost_total), 0) AS total_talent_cost,
      COALESCE(SUM(bt.client_charge_total), 0) AS total_client_revenue,
      COALESCE(SUM(bt.gross_profit), 0) AS gross_profit
