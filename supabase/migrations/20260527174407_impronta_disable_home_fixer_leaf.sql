...
-- is only created by 20260909221926, which sorts after this file). Guard the
-- disable + its audit row together as one unit — skip both, or do both,
-- never log a disable that did not happen. No-op here; unchanged behaviour
...
BEGIN
  IF to_regclass('public.agency_taxonomy_settings') IS NOT NULL THEN
    ...
    WHERE a.slug = 'impronta'
      AND t.slug = 'home-fixer';
  ELSE
    RAISE NOTICE 'agency_taxonomy_settings absent — skipping home-fixer disable (created by 20260909221926 later in the same replay)';
  END IF;
END $$;
